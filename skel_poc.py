...
def _fit_skel_sequence(torch, model, motion, seed_fit, start_index=0, iterations=16, progress_cb=None, empirical_profile=None, acquisition_mode="monocular_depth_safe"):
    ...
    result={'version':'110.3.22.1-segment-gentle-anatomical-layers','retargeting_mode':'fullbody_segment_frame_reference_skel_skeleton_layer','scale':float(scale.cpu()),
...
def _skel_volumetric_anatomy_2010(joints, joint_names):
    """V110.3.22.1 · Generates volumetric anatomically-shaped bone and muscle meshes.
...
def _plot_skel_mesh_walker(mesh_seq):
    """V110.3.22.1 · visor de atlas anatómico REAL sobre SKEL. Sin fallback geométrico."""
    try:
        ...
        st.markdown("---")
        st.markdown("## 🧠 Atlas musculoesquelético de alta fidelidad · Visible Human")
        st.caption(
            "Nueva rama V110.3.22.1. El objetivo es sustituir progresivamente la geometría visual Hamner "
            "por tejidos segmentados de Visible Human. La primera integración usa el conjunto de miembro inferior "
            "de la University of Denver: hueso, cartílago, ligamento y 76 músculos por sujeto."
        )
        ...
        if _vh_manifest and _vh_manifest.get("ready"):
            ...
            _vh_frame=st.slider(
                "Frame de validación anatómica",
                1,int(len(Vviz)),1,1,key="v3220_frame"
            )-1
            _vhc1,_vhc2,_vhc3=st.columns(3)
            _vh_skin=_vhc1.checkbox("🧍 Piel SKEL",True,key="v3220_skin")
            _vh_bones=_vhc2.checkbox("🦴 Huesos reales inferiores",True,key="v3220_bones")
            _vh_muscles=_vhc3.checkbox("💪 Músculos volumétricos reales inferiores",True,key="v3220_muscles")

            try:
                import plotly.graph_objects as _go
                _Jvh=np.asarray(mesh_seq.get("joints"),dtype=np.float32)
                _JNvh=[str(x) for x in mesh_seq.get("joint_names",[])]
                _parts_vh=build_visible_human_frame(
                    _vh_manifest,_Jvh,_JNvh,np.asarray(Vviz[_vh_frame],dtype=np.float32),
                    frame=_vh_frame,show_bones=_vh_bones,show_muscles=_vh_muscles,
                    max_faces_each=260
                )
                _fg=_go.Figure()
                if _vh_skin:
                    _vs=np.asarray(Vviz[_vh_frame],dtype=np.float32)
                    _fs=np.asarray(F[:,:3],dtype=np.int32)
                    _ss=max(1,int(np.ceil(len(_fs)/4200.0)))
                    _fs=_fs[::_ss]
                    _fg.add_trace(_go.Mesh3d(
                        x=_vs[:,0],y=_vs[:,1],z=_vs[:,2],
                        i=_fs[:,0],j=_fs[:,1],k=_fs[:,2],
                        opacity=.13,name="Piel SKEL",showscale=False
                    ))
                for _pp in _parts_vh:
                    _vv=np.asarray(_pp["V"]); _ff=np.asarray(_pp["F"])
                    _fg.add_trace(_go.Mesh3d(
                        x=_vv[:,0],y=_vv[:,1],z=_vv[:,2],
                        i=_ff[:,0],j=_ff[:,1],k=_ff[:,2],
                        opacity=.98 if _pp["kind"]=="bone" else .58,
                        name=_pp["name"],showscale=False
                    ))
                _fg.update_layout(
                    title=f"Visible Human real · miembro inferior · frame {_vh_frame+1}/{len(Vviz)}",
                    height=820,margin=dict(l=0,r=0,t=45,b=0),
                    scene=dict(aspectmode="data")
                )
                st.plotly_chart(_fg,use_container_width=True,key="v3220_vh_preview")
                st.info(
                    "Esta capa usa mallas musculoesqueléticas segmentadas de Visible Human para el miembro inferior. "
                    "El registro sigue siendo una estimación anatómica sobre SKEL; no es la anatomía individual del paciente."
                )
            except Exception as _e:
                st.warning(f"No se pudo construir la vista Visible Human: {type(_e).__name__}: {_e}")
        else:
            st.info(
        ...
        st.markdown("## 🧍🦴💪 Visor anatómico 3D · SKEL + OpenSim Hamner · registro optimizado")
        st.caption(
            "Interfaz única estilo V110.3.20.9. Piel, esqueleto y músculo comparten los mismos 75 frames. "
            "BodyParts3D, Rajagopal y visores previos quedan retirados del flujo activo."
        )
        ...
        if not _ham_status.get("ready"):
            ...
            _cache_key=(int(len(_Sv)),int(_Sv.shape[1]),float(np.nanmean(_Jv[0])))
            if st.session_state.get("v32110_seq_key")!=_cache_key:
                with st.spinner("Preparando las tres capas para los 75 frames…"):
            ...
            _payload=_json.dumps({
                ...
                "M":_mus_frames,"ids":[int(x) for x in ids],"ranges":_ranges
            },separators=(",",":"))
            _js=_get_plotlyjs()
...
<div id='bar'>
  ...
  <button id='multiBtn' onclick='toggleMulti()'>🎥 Multiplano</button>
  <button id='recBtn' onclick='recordCurrentView()'>⬇ Descargar vídeo</button>
  <b id='counter'>Frame 1/{len(ids)}</b>
  <input id='slider' type='range' min='0' max='{max(len(ids)-1,0)}' value='0'>
</div>
<div id='status'>Gira, inclina y acerca el modelo directamente con el ratón. La descarga conserva esa vista.</div>
<div id='plot'></div>
<script>
const P={_payload};
...
let s=xyz(P.SV,0),b=xyz(P.BV,0),m=mus(0);
const data=[
  ...
  {{type:'scatter3d',mode:'lines',x:m[0],y:m[1],z:m[2],name:'Músculo',
    line:{{width:9}},visible:true}}
];
...
window.layer=function(mode){{
  ...
  Plotly.restyle(gd,{{visible:['muscle','bone-muscle','all'].includes(mode)}},[2]);
}};

function frame(k){{
  n=(k+P.SV.length)%P.SV.length;
  s=xyz(P.SV,n);b=xyz(P.BV,n);m=mus(n);
  Plotly.restyle(gd,{{x:[s[0]],y:[s[1]],z:[s[2]]}},[0]);
  Plotly.restyle(gd,{{x:[b[0]],y:[b[1]],z:[b[2]]}},[1]);
  Plotly.restyle(gd,{{x:[m[0]],y:[m[1]],z:[m[2]]}},[2]);
...
async function recordCurrentView(){{
  ...
  const wasOn=on; on=false; cancelAnimationFrame(raf);
  const startFrame=n;
  const cam0=currentCamera();
  const multi0=multiplano;
  st.textContent='Preparando descarga de la vista actual…';

  try{{
    /* Render plot to images into a dedicated recording canvas. */
    const W=960,H=960;
    ...
    for(const x of candidates){{if(MediaRecorder.isTypeSupported(x)){{mime=x;break}}}}
    const chunks=[];
    ...
    for(let kk=0;kk<P.SV.length;kk++){{
      frame(kk);
      /* in free mode keep the exact camera the user left on screen */
      if(!multi0 && cam0) await Plotly.relayout(gd,{{'scene.camera':cam0}});
      ...
      ctx.fillStyle='white';ctx.fillRect(0,0,W,H);ctx.drawImage(im,0,0,W,H);
      await new Promise(r=>setTimeout(r,40)); /* ~25 fps recorder cadence */
      st.textContent='Generando vídeo '+(kk+1)+'/'+P.SV.length+'…';
    }}
    ...
    a.download='PhysioSentinel_'+currentLayer+'_'+(multi0?'multiplano':'vista_actual')+'.'+ext;
    a.click();
    setTimeout(()=>URL.revokeObjectURL(a.href),10000);
    st.textContent=ext==='mp4'
      ? '✓ Vídeo MP4 descargado con la orientación mostrada.'
      : '✓ Vídeo descargado. Este navegador ha usado WebM porque MediaRecorder MP4 no está disponible.';
  }}catch(e){{
    console.error(e);
    st.textContent='No se pudo generar el vídeo en el navegador: '+e;
  }}finally{{
    frame(startFrame);
    if(!multi0 && cam0) Plotly.relayout(gd,{{'scene.camera':cam0}});
    if(wasOn)play(speed);
    recBtn.disabled=false;
            ...
            st.markdown("### 📊 Pestaña 12")
            st.caption(
                "Se conservan curvas articulares, ROM, derivadas temporales, suavidad, simetría, coordinación, "
                "diagramas ángulo–ángulo y trayectorias 3D, también al cargar un NPZ existente."
            )
    except Exception as exc:
...
def _load_mesh_sequence_npz_bytes_2012(raw):
    """V110.3.22.1 · Reabre un NPZ SKEL ya calculado sin volver a generar los 75 frames."""
    try:
...
def render_skel_poc_panel(motion, acquisition_mode="monocular_depth_safe", acquisition_label=None):
    ...
    st.caption("V110.3.22.1 · Carga el NPZ descargado de este registro para abrir directamente sus 75 frames SKEL. No repite fitting, retargeting ni generación de los 75 frames.")
